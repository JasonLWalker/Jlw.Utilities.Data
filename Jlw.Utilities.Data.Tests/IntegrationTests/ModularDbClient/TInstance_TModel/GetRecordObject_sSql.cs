...
namespace Jlw.Utilities.Data.Tests.IntegrationTests.ModularDbClient.TInstance_TModel
{
    ...
    public class GetRecordObject_sSql : SqlLocalDbInstanceFixtureBase<ModularDataRepository<ITestDataModel, TestDataModel>>
    {
        ...
        public void Should_Succeed_ForSqlQuery()
        {
            ...
            // Act
            var response = SqlClient.GetRecordObject<ITestDataModel, TestDataModel>(ConnectionString, "Select TOP 1 * FROM TestTable WHERE Id = '1';", new[] {new KeyValuePair<string, object>("ID", "1")}, false);
        ...
        public void Should_Succeed_ForStoredProcedure()
        {
            ...
            // Act
            var response = SqlClient.GetRecordObject<ITestDataModel, TestDataModel>(ConnectionString, "sp_GetRecordData", new[] { new KeyValuePair<string, object>("ID", "2") }, true);
        ...
        public void Should_ThrowArgumentException_ForEmptySqlQuery()
        {
            ...
            {
                var response = SqlClient.GetRecordObject<ITestDataModel, TestDataModel>(ConnectionString, "", new[] { new KeyValuePair<string, object>("ID", "3") }, false);
            });
        ...
        public void Should_ThrowArgumentException_ForEmptyStoredProcedure()
        {
            ...
            var ex = Assert.ThrowsException<ArgumentException>(() =>
            {
                var response = SqlClient.GetRecordObject<ITestDataModel, TestDataModel>(ConnectionString, "", new[] { new KeyValuePair<string, object>("ID", "3") }, true);
            });
