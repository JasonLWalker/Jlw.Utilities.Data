...
{
    [TestClass]
    public class GetRecordObject_definition_withcallback : SqlLocalDbInstanceFixtureBase<ModularDataRepository<ITestDataModel, TestDataModel>>
    {
        ...
        protected RepositoryRecordCallback<TestDataModel> RecordCallback = (o) => new TestDataModel()
        {
        ...
        public void Should_Succeed_ForSqlQuery_StringArray()
        {
            ...
            var definition = new RepositoryMethodDefinition<TestDataModel, TestDataModel>("SELECT TOP 1 * FROM TestTable WHERE Id = @id", new string[] { "Id" }, RecordCallback);

            var expected = new TestDataModel() { Id = 1, Name = "Test User", Description = "This is a test user", LastUpdated = DateTime.Now };

            // Act
            var response = SqlClient.GetRecordObject<TestDataModel>(new TestDataModel(){Id=1}, ConnectionString, definition);
        ...
        public void Should_Succeed_ForSqlQuery_Kvp()
        {
            // Arrange
            base.InitializeInstanceData(SqlInitFilename);
            var definition = new RepositoryMethodDefinition<TestDataModel, TestDataModel>("SELECT TOP 1 * FROM TestTable WHERE Name = @name AND Id=@id", new [] { new KeyValuePair<string, object>("Id", 1),  new KeyValuePair<string, object>("Name", "Test User") }, RecordCallback);

            var expected = new TestDataModel() { Id = 1, Name = "Test User", Description = "This is a test user", LastUpdated = DateTime.Now };

            // Act
            var response = SqlClient.GetRecordObject<TestDataModel>(null, ConnectionString, definition);
        ...
        public void Should_Succeed_ForStoredProcedure_StringArray()
        {
            // Arrange
            base.InitializeInstanceData(SqlInitFilename);
            var definition = new RepositoryMethodDefinition<TestDataModel, TestDataModel>("sp_GetRecordData", CommandType.StoredProcedure, new string[] { "Id" }, RecordCallback);
            var expected = new TestDataModel() { Id = 2, Name = "Test User 2", Description = "This is another test user", LastUpdated = DateTime.Now };

            // Act
            var response = SqlClient.GetRecordObject<TestDataModel>(new TestDataModel(){Id = 2}, ConnectionString, definition);
        ...
        {
            // Arrange
            var definition = new RepositoryMethodDefinition<TestDataModel, TestDataModel>("", new string[] { "Id" }, RecordCallback);

            // Act / Assert
            var ex = Assert.ThrowsException<ArgumentException>(() =>
            {
                var response = SqlClient.GetRecordObject<TestDataModel>(new TestDataModel() { Id = 3 }, ConnectionString, definition);
            });
        ...
        {
            // Arrange
            var definition = new RepositoryMethodDefinition<TestDataModel, TestDataModel>("", CommandType.StoredProcedure, new string[] { "Id" }, RecordCallback);
        ...
        {
            // Arrange
            var definition = new RepositoryMethodDefinition<TestDataModel, TestDataModel>("sp_GetRecord", CommandType.StoredProcedure, new string[] { "Id" }, RecordCallback);
